...
type (
	ListenerTLSConfig = gwv1.ListenerTLSConfig
	Group             = gwv1.Group
	Hostname          = gwv1.Hostname
	Kind              = gwv1.Kind
	ObjectName        = gwv1.ObjectName
	ProtocolType      = gwv1.ProtocolType
	RouteNamespaces   = gwv1.RouteNamespaces
	RouteGroupKind    = gwv1.RouteGroupKind
	SectionName       = gwv1.SectionName
	Namespace         = gwv1.Namespace
)

// PortNumber defines a network port that allows 0 to indicate dynamic
// port assignment. This type should only be used in spec fields where port 0
// has the special meaning of "auto-assign a port".
type PortNumber int32

// +kubebuilder:rbac:groups=enterprise.solo.io,resources=enterpriselistenersets,verbs=get;list;watch;update
// +kubebuilder:rbac:groups=enterprise.solo.io,resources=enterpriselistenersets/status,verbs=get;update;patch

// +genclient
// +kubebuilder:object:root=true
// +kubebuilder:resource:categories={enterprise,es},path=enterpriselistenersets,shortName=elset
// +kubebuilder:subresource:status
// +kubebuilder:metadata:labels={app=enterprise,app.kubernetes.io/name=enterpriselistenerset}
// +kubebuilder:storageversion
// +kubebuilder:printcolumn:name="Accepted",type=string,JSONPath=`.status.conditions[?(@.type=="Accepted")].status`
// +kubebuilder:printcolumn:name="Programmed",type=string,JSONPath=`.status.conditions[?(@.type=="Programmed")].status`
// +kubebuilder:printcolumn:name="Age",type=date,JSONPath=`.metadata.creationTimestamp`

// EnterpriseListenerSet defines a set of additional listeners to attach to an existing Gateway.
...
type EnterpriseListenerSet struct {
	...
	// +required
	Spec EnterpriseListenerSetSpec `json:"spec"`
	...
	// +optional
	Status EnterpriseListenerSetStatus `json:"status,omitempty"` // nolint:kubeapilinter // optionalfields - allow status to be a non-pointer
}

// EnterpriseListenerSetSpec defines the desired state of a ListenerSet.
type EnterpriseListenerSetSpec struct {
	...
	// +required
	Listeners []EnterpriseListenerEntry `json:"listeners"`
}

type EnterpriseListenerEntry struct {
	...
	Port PortNumber `json:"port,omitempty"` // nolint:kubeapilinter // optionalfields - allow Port to be a non-pointer
...
type EnterpriseListenerSetStatus struct {
	...
	// +listMapKey=name
	// +kubebuilder:validation:MaxItems=64
	Listeners []EnterpriseListenerEntryStatus `json:"listeners,omitempty"`
}

// ListenerStatus is the status associated with a Listener.
type EnterpriseListenerEntryStatus struct {
...
// +kubebuilder:resource:categories={enterprise,ee},path=enterpriselistenersets
// +kubebuilder:subresource:status
// +kubebuilder:metadata:labels={app=enterprise,app.kubernetes.io/name=enterpriselistenersets}
type EnterpriseListenerSetList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []EnterpriseListenerSet `json:"items"`
}

// GetListeners ensures that kgateway can fetch the EnterpriseListenerSet listeners even though it does not have access to the type
// This allows it to implement the krtcollections.ListenerCollection interface

func (g *EnterpriseListenerSet) GetListeners() []gwv1.Listener {
	listeners := make([]gwv1.Listener, len(g.Spec.Listeners))
...
func ToListener(listenerEntry EnterpriseListenerEntry) gwv1.Listener {
	duplicate := listenerEntry.DeepCopy()
	upstreamListener := gwv1.Listener{
		Name:     duplicate.Name,
	...
	if duplicate.AllowedRoutes != nil {
		upstreamListener.AllowedRoutes = &gwv1.AllowedRoutes{
			Namespaces: duplicate.AllowedRoutes.Namespaces,
			Kinds:      duplicate.AllowedRoutes.Kinds,
		}
	}
